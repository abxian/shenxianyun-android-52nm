...
- 检查 APK 更新。
- 向后台上报在线设备。
- 在线心跳使用 120–140 秒抖动间隔；失败状态写入本地且只保留最新一条，恢复后由下一轮覆盖上报。
...
## 15. 商业化设备身份与流量上报（2026-08-24）

- `client_id` 仍是安装实例 UUID，升级时保留；不要用设备名称、IP 或 UA 替代。
- 安全导入时额外发送 `device_key`。它由 `packageName + ANDROID_ID` 在本机 SHA-256 后生成，原始 `ANDROID_ID` 不出端；无法取得时回退到受备份保护的随机值。
- 服务端会再次 HMAC 并生成 `canonical_device_id`，用于重装后的设备归一与绑定名额。客户端不自行合并历史身份。
- 流量来自 Mihomo 原始累计上/下行字节，使用持久化的 `counter_id + sequence` 幂等上报。服务端确认前不得推进已确认基线。
- 正常上报间隔为 5 分钟并叠加 0–60 秒抖动；失败指数退避，最长 30 分钟；启动后首报随机延迟 5–30 秒，避免集中惊群。

发布前除原清单外还要验证：同一手机升级不新增设备、重装后重新导入仍只占一个归一设备名额、另一台真实手机会触发绑定上限、上下行方向与 Mihomo 原始计数一致、服务端 429/503 后累计量最终补报且不重复。
