...
- 在线统计
  ...
  - 连接期间约每 120–140 秒上报一次并加入随机抖动；失败时本地只保留最后状态，后续心跳恢复上报，不批量重放历史心跳。
- 一键导入
...
- 正式资产只进入 52nm 独立下载链（Dufs 目录 `/52nm`）并只更新 52nm `/api/app-version`；禁止写入神仙云 `/sxy`、哪吒安装源 `/jcjc` 或 sxnn 接口。
...
52nm 后台 APK 更新配置里的 `latest_version_code` 要使用 `211049` 或更高。
